...
8. 如果 connector 已经存在，但缺一个反复需要的平台动作，我知道用 **`connector_behavior_add`** 先登记 behavior capability，而不是改核心代码或硬编码 endpoint。
9. 我知道用 **`status` / `credential` / `quiet`** 从工具 JSON 验收，而不是一上来盲跑。
...
- `connector_behavior_add`：把新发现的 connector 行为登记到既有 manifest 的 `capabilities[]`，不授予执行权限，也不写凭据或 trust allowlist
