...
2.  **深度阅读设计文档**:
    ...
    - 读取 `{TARGET_DIR}/04_SYSTEM_DESIGN/`（如存在；**当前契约**仅包含非归档、非过程稿：排除 `*.old.md`、文件头声明为 **Non-Contract Archive** 的文件、`_archive/` / `_legacy/`；`04_SYSTEM_DESIGN/_review/` 默认不当作契约源，除非用户或本轮审查明确要求）
