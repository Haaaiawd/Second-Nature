...
1. **加载文档 (必须)**: 读取 `02_ARCHITECTURE_OVERVIEW.md`、`04_SYSTEM_DESIGN/` 下**当前契约**系统设计（含同域 `*.detail.md`）以及 `03_ADR/` 下全部 ADR。**排除**契约源污染：`*.old.md`、文件头声明为 **Non-Contract Archive** 的文件、`_archive/` / `_legacy/`（若存在）；`04_SYSTEM_DESIGN/_review/` 默认视为过程稿，不纳入当前契约闭包，除非用户或任务明确要求。
