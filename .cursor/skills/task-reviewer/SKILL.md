...
1. **加载文档 (必须)**: 读取 `.anws/v{N}/05_TASKS.md`、`01_PRD.md`、`02_ARCHITECTURE_OVERVIEW.md`、所有 `03_ADR/*.md`，以及 `04_SYSTEM_DESIGN/` 下**当前契约**设计文档（如存在；排除规则同 design-reviewer：`*.old.md`、Non-Contract Archive、`_archive/` / `_legacy/`；`_review/` 默认排除除非任务或用户明确要求）。
...
- **契约证据依赖**: Pass G 默认依赖 `04_SYSTEM_DESIGN/` 下**当前契约**文档（排除规则见「任务目标」第 1 条）。若任务涉及公共契约而设计文档缺失，必须报告“证据不足 / 契约定义缺口”，不得静默忽略。
