...
connector 的行为可以在初始化之后继续长出来。如果 Claw 反复需要某个 manifest 里还没有的动作，可以用 `connector_behavior_add` 把 capability declaration 追加到 `.second-nature/connectors/{platformId}/manifest.yaml`。这只是把行为记下来：真正执行仍然要走 registry reload、route planning、凭据、幂等和 trust policy。
