...
- **Execution（Cursor / CI）**: `NOT RUN` — 未连接你的 OpenClaw；以下步骤供你在目标环境执行并回填证据。
...
| J2 | **显式 CLI `heartbeat_check`**：在配置好 `workspaceRoot` 的前提下调用插件暴露的 `heartbeat_check`（OpenClaw command / tool 文档中的调用方式），得到结构化 `HeartbeatSurfaceResult`（非仅 `runtime_carrier_only`，若你的环境满足 full runtime） | NOT RUN | 完整 JSON 或 redacted 摘要 | 若仅 `runtime_carrier_only`，在证据中记 **unknown** 并抄 `reasons` |
