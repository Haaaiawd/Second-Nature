...
Connector behavior can evolve after init. If Claw repeatedly needs an action that is not in a connector manifest yet, `connector_behavior_add` appends a capability declaration to `.second-nature/connectors/{platformId}/manifest.yaml`. Each new behavior needs a reviewable motive via `description` or `sourceRefs`; repeated observations can be recorded with `observedCount`. This is only a note in the manifest: execution still goes through registry reload, route planning, credentials, idempotency, and trust policy.
