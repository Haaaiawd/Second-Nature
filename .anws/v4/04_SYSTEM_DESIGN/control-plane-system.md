...
- 当前 heartbeat shipping bridge contract 已选定为 `HEARTBEAT.md + second_nature_ops("heartbeat_check")`
- `src/core/second-nature/runtime/service-entry.ts` 仍只是最小 runtime handle，不承担 per-heartbeat callback 语义
- 目前“用户明确任务是否受节律影响”已经形成运行合同，但宿主真实桥接与平台真实出口验证仍待 S3 收口
...
- `HEARTBEAT.md + second_nature_ops("heartbeat_check")` 是当前选定的 shipping bridge contract；`HEARTBEAT.md` 负责宿主提示，`heartbeat_check` 负责把轮次转成可消费 signal/result
- service surface 仍可作为 runtime carrier / helper，但不能单独替代 shipping bridge contract
...
| --- | --- | --- | --- |
| `HostHeartbeatBridge` | 将 OpenClaw heartbeat 语义桥接成可被 control-plane 消费的运行信号 | TypeScript / prompt-bridge / host integration | shipping contract 为 `HEARTBEAT.md + second_nature_ops("heartbeat_check")` |
...
**当前 shipping bridge 路径**:
- `HEARTBEAT.md + tool use`：由 heartbeat 轮中的 LLM 读取 `HEARTBEAT.md` 并显式调用 `second_nature_ops` 的 `heartbeat_check`
- `command parity`：允许等价 `second-nature heartbeat_check` 命令作为调试/验证入口，但其语义必须与 tool surface 对齐

**保留的辅助路径**:
- `service-assisted bridge`：由 plugin service 提供稳定 runtime 状态与 bridge helper，但不假装自己收到 per-heartbeat callback
- 其他宿主可实现方案：允许后续探索，但不得改变当前 shipping contract 的验收语义
...
### 11.3 宿主接入验证
- OpenClaw heartbeat 实际能否通过 `HEARTBEAT.md + heartbeat_check` 唤醒 runtime entry
- heartbeat 轮静默时是否符合宿主 `HEARTBEAT_OK` 协议
- 用户明确任务是否绕开 rhythm gate
- `HEARTBEAT.md` 与 `heartbeat_check` surface 是否稳定对齐，并能产出可消费的结构化结果，而不是停留在文字约定层
...
- heartbeat 主生命线依赖宿主 heartbeat 语义，当前 shipping bridge contract 为 `HEARTBEAT.md + second_nature_ops("heartbeat_check")`
- `HEARTBEAT.md` 是宿主侧行为指导文件，`heartbeat_check` 是当前 shipping plugin surface；两者必须同步演进
- observability 需要能区分：
  - heartbeat decision
  - user task route
