...
| 5 | [接口设计](#5-接口设计-interface-design) | command / tool / service / manifest 合同 |
...
- `plugin/index.ts` 当前负责注册 command / tool / service
- command router 真正实现在 `src/cli/index.ts`
- wrapper 当前尝试通过相对路径加载源码仓中的运行时代码
- npm 发布包只带 wrapper 和 manifest，安装后 runtime 依赖图不闭合
...
- OpenClaw 原生插件天然支持 manifest/schema 与 runtime registration 分层
- 发布包当前失败的根因不是 OpenClaw 限制，而是我们 artifact 边界没收好
- 最稳的路线是保留 wrapper，但只允许它引用包内 runtime artifact
- fallback 值得保留，但不能继续当正式运行模式
- jiti、`npm install --ignore-scripts` 与 `better-sqlite3` 等原生依赖是当前 packaging 风险最高的部分，必须先做 feasibility POC
...
| `Command Router` | 聚合 `status / policy / credential / quiet / report / session / explain` | TypeScript | 与 tool shell 共用同一 router |
| `CLI Runtime Dependencies` | 创建 stateDb / observabilityDb / stateApi / readModels / actionBridge | TypeScript | 这是发布包必须闭合的最小依赖图 |
| `Service Surface` | 暴露 `second-nature-runtime` 与 lifecycle service | TypeScript | 可作为 heartbeat host bridge 候选载体之一 |
...
| `startRuntimeService()` | [REQ-015] | service surface 已注册 | host service start context | 启动 heartbeat host bridge 候选路径 | 待 `/forge` |
...
interface PackagedCommandSurface {
  commandName: 'status' | 'policy' | 'credential' | 'quiet' | 'report' | 'session' | 'audit' | 'explain';
  available: boolean;
}
```
...
- `createStateDatabase()` / `createStateAPI()`
- `createObservabilityDatabase()`
- `createCliReadModels()`
- `createActionBridge()`
- OpenClaw plugin registration API
- `better-sqlite3` / SQLite 驱动兼容路径（待 packaging feasibility POC 验证）
...
- runtime service / tool / prompt bridge 至少有一条路径能承载 heartbeat bridge
...
- 原生依赖路径在目标宿主上要么可工作，要么已有明确替代策略；不得留在未验证状态
