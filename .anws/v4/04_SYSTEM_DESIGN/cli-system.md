...
- wrapper 当前已经改为解析包内 runtime，而不是继续走源码仓相对路径
- `second_nature_ops` 与 `second-nature` command 已形成 host-safe 最小 surface，但当前 shipping bridge contract `heartbeat_check` 仍未进入该 surface
- npm 发布包已能携带 runtime artifact，但 heartbeat host bridge 仍未随 surface 一起闭合
...
- 发布包当前失败的根因已经不再是“完全没有 runtime artifact”，而是 shipping surface 还没有把 heartbeat host bridge contract 收进口
- 最稳的路线仍是保留 wrapper，但只允许它引用包内 runtime artifact
- fallback 值得保留，但不能继续当正式运行模式
- 当前更真实的宿主闭环缺口是 `HEARTBEAT.md + heartbeat_check` 尚未随 plugin surface 一起交付
