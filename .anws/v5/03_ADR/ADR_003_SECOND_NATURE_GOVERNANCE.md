...
> v5 延伸注记（2026-05-01）:
> 本 ADR 的 Quiet、Narrative Reflection 与用户主动联系原则继续有效，但 v5 将“真实事实”进一步收紧为 source-backed life evidence。
> Quiet 和记忆整理必须消费 `LifeEvidence` / `WorkLifeEvent` / `PlatformLifeEvent` 等可追溯输入；主动联系必须经过 `OutreachJudgment`、cooldown、dedupe 与 delivery availability 检查。
> “不做僵硬分类”不表示没有阈值和审计，而是不把表达体验退化为客服工单树。
