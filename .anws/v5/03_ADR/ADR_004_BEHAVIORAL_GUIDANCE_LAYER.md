...
> v5 延伸注记（2026-05-01）:
> Behavioral Guidance 在 v5 中可以生成 friend-like outreach draft，但仍不拥有行动决策权、投递权或事实真相源管理权。
> 它必须读取 control-plane 允许后的 evidence refs 与 user interest snapshot，生成短、自然、有来由的表达；不得编造平台经历、用户偏好或 delivery 状态。
