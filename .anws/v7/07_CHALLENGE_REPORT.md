...
### High / Medium 发现修复记录（2026-05-21）

全部 35 条 High/Medium 发现已完成 L0 文档级修复：

| DR ID | 严重度 | 修复位置 | 修复内容摘要 |
|-------|--------|----------|-------------|
| DR-003 | High | `body-tool-system.md` §4.3 | sequenceDiagram 补充 `invalidateAffordanceCache` 节点；probe result 写入后立即失效对应 connectorId 的 affordance 缓存 |
| DR-004 | High | `body-tool-system.md` §5.1 | 新增 AffordanceContextScope 语义说明注释块（platformIds 白名单、goalKind 影响过滤、allowedStatuses 默认值） |
| DR-005 | High | `body-tool-system.md` §4.5（新增） | 新增 BehaviorPromotion 状态机 mermaid 图：candidate → approved/rejected/expired；幂等 approve、7 天 expired、可重新提交 |
| DR-006 | High | `body-tool-system.md` §5.1 + `connector-system.md` §5.1 | body-tool 触发 probe 前验证 `safe_for_probe: true`；connector WetProbeRunner 强制校验，失败返回 `StructuredUnavailableReason{probe_policy_denied}` |
| DR-007 | Medium | `body-tool-system.md` §6.1 | `ToolExperienceRow.failureClass` 注释明确：直接从 `ConnectorResult.failureClass` 转写，body-tool 不自行推导 |
| DR-008 | Medium | `body-tool-system.md` §10.2 | 新增 AffordanceAssembler 缓存策略说明（heartbeat-cycle TTL + breaker/probe/registry 变更时失效） |
| DR-009 | Medium | `body-tool-system.md` §4.3 | sequenceDiagram 末尾补充 auto-probe 写入 state-memory 并被 assembleAffordanceMap 读取的时序节点 |
| DR-010 | Medium | `body-tool-system.md` §5.1 + §5.2 | `recordExperience` 入参新增 `triggerSource`；§5.2 接口注释说明 control-plane 传 `heartbeat`，runtime-ops 传 `manual_run` |
| DR-014 | High | `state-memory-system.md` §6.2 | AgentGoal.kind/scope 格式约束：kind snake_case lowercase 枚举、scope null 表全局、replace 等值判断规则 |
| DR-015 | High | `state-memory-system.md` §4.4 | Goal lifecycle 状态机补充 paused 完整出边：paused → completed/expired/replaced/active |
| DR-016 | High | `control-plane-system.md` §4.2 | EmbodiedContextAssembler 边界约束：最多 7 个 read port 调用、P95 < 400ms、职责严格限定为"读取并组装" |
| DR-017 | High | `state-memory-system.md` §6.2 | RestoreSnapshot entity 白名单：可快照 6 类实体，excludedSensitiveKinds 默认值，Dream/Diary 只存 ref |
| DR-018 | High | `state-memory-system.md` §12 | 新增 Schema Migration 策略子章节：`_meta` 版本号、事务迁移、失败标记 degraded、无 down-migration |
| DR-019 | High | `state-memory-system.md` §12 | 新增 Write Queue 与并发保护子章节：串行 write queue、BEGIN EXCLUSIVE、50ms 退避重试 3 次 |
| DR-020 | Medium | `control-plane-system.md` §4.2 | EmbodiedContext 各 slice trim 策略：recentInteractions LIFO 10 条、experiences 10 条、sourceRefs 20 条去重 |
| DR-021 | Medium | `state-memory-system.md` §5.1 | upsertAgentGoal 事务语义：BEGIN EXCLUSIVE、先到胜出、后到执行 replace 语义 |
| DR-022 | Medium | `state-memory-system.md` §4.2 | WriteValidationGate 强制范围：所有写入路径必须经过，拒绝条件和错误类型 `write_validation_failed:{reason}` |
| DR-024 | High | `dream-quiet-system.md` §5.1 | loadAcceptedProjection 降级语义：空数组返回 + `context_degraded:dream_projection_unavailable` reason code |
| DR-025 | High | `dream-quiet-system.md` §2.1 [G1] | fact claim sourceRefs 声明为 `[string, ...string[]]` non-empty tuple；ClaimSynthesizer 单测接缝 |
| DR-026 | High | `dream-quiet-system.md` §10.2 | Dream lock 下 claims 幂等加载机制：loadDreamInputs 查询"未被 accepted projection 引用的 claims"，自然幂等去重 |
| DR-027 | High | `dream-quiet-system.md` §9.4 | ModelAssistPort `RedactedEvidenceBundle` 品牌类型强制；RedactionGate.redactBundle() 为必要前置调用 |
| DR-028 | High | `guidance-voice-system.md` §5.1 | draft delivery 前 `validateDraftSources` 重验证；evidence 删除/redact 后 draft 标记 `invalid`，返回 `draft_source_invalidated` |
| DR-029 | Medium | `guidance-voice-system.md` §10 | ChannelFeedbackIngestion 写入失败处理：指数退避重试 3 次（500/1000/2000ms），失败写 audit event，禁止静默丢失 |
| DR-030 | Medium | `guidance-voice-system.md` §5.2 | 新增 `GuidanceDraftRequest` TypeScript 接口定义（requestId/sceneKind/evidencePackRef/relationshipContextRef 等 7 字段） |
| DR-031 | Medium | `dream-quiet-system.md` §11 | Language Quality Checklist：3 条 rule-based lint 规则（无干燥白话/有具体锚点/无过度阐释），`style_lint_failed` 降级标记 |
| DR-033 | High | `observability-health-system.md` §4.2 | AppendOnlyAuditStore in-memory `lastHashCache`：O(1) hash chain 写入，per-family 缓存，进程重启后 DB 回填 |
| DR-034 | High | `observability-health-system.md` §6.1 + `AGENTS.md` | RuntimeSecretAnchorView 新增 `recoverySteps: RecoveryStep[]` 内联恢复步骤；AGENTS.md 新增 `## Bootstrap Recovery` 章节 |
| DR-035 | High | `observability-health-system.md` §3.3 | 各系统 audit write 失败策略约定：state-memory 默认 fire-and-forget、高安全性操作可 fail-fast、connector execution 继续返回结果 |
| DR-036 | High | `observability-health-system.md` §5.1 | 探针独立超时配置表（env 200ms / cron 500ms / secret 1000ms 等）；总体上限 3000ms；全部超时返回 `lastKnownAt` |
| DR-037 | High | `observability-health-system.md` §5.1 | NarrativeTimeline cursor 分页、最大查询范围 90 天、超出返回 error（不 truncate） |
| DR-038 | Medium | `runtime-ops-system.md` §4.2 | ManualRunDispatcher 并发控制：write queue 串行化、triggerSource 由调用方写入、不阻塞 cron heartbeat |
| DR-039 | Medium | `observability-health-system.md` §5.1 | audit hash chain 损坏 5 步处理：隔离 segment / 标记 degraded / 触发 operator alert / 下游 warnings / 无自动修复 |
| DR-040 | Medium | `observability-health-system.md` §6.1 | audit family/plane 注册机制：`audit-family-registry.json` + 8 系统已注册 family 表格，新增系统必须注册 |
| DR-041 | Medium | `observability-health-system.md` §5.1 | restore 原子性：audit 失败不回滚 state（fire-and-forget）；partial_restore_error 记录已完成/未完成 entity 清单 |
| DR-042 | Medium | `runtime-ops-system.md` §6.1 | 新增 `SelfHealthView` 完整 schema（overall/generatedAt/degraded_dimensions/dimensions/lastKnownAt）；1:1 投影 observability SelfHealthSnapshot |
