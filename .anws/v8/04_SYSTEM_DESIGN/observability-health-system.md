...
Stage-level `degraded` is intentionally not allowed. `degraded` is an aggregate overall status derived from precise stage states.

Evidence level is derived by the shared `EvidenceLevelClassifier`; `CausalLoopHealthSnapshot.evidenceLevel` must be the minimum evidence level across required stage proofs for the reported cycle.
