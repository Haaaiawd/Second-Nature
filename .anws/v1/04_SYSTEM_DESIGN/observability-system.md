...
- 凭据脱敏（日志中不记录敏感信息）
- 对事件 payload 执行最小化与白名单裁剪，避免私信、正文、任务全文等业务内容落盘

**不负责**:
- 不做实时告警（首版超出范围）
- 不做复杂数据分析（只提供原始数据+基础聚合）
- 不存储长期记忆内容、私信正文、社区正文或任务全文（只存审计元数据与最小解释字段）
...
- **性能约束**: 写入不影响主流程（异步）
- **隐私约束**: 凭据脱敏，导出仅包含最小化后的审计数据，不导出原始未裁剪 payload
...
| `minimizePayload(event)` | 原始事件 | `AuditPayload` | 丢弃未授权内容字段，仅保留白名单元数据 |
...
| 实体 | 关键字段 |
|------|---------|
| **EventLog** | `id`, `timestamp`, `type`, `platformId?`, `sessionId?`, `payload`（仅允许白名单审计字段） |
...
| 脱敏 | 白名单裁剪 + 精确字段脱敏 | 仅黑名单正则（rejected: 无法阻止正文类内容落盘） |
...
## 9. 安全性考虑 (Security Considerations)
 ...
 - 导出仅包含脱敏且最小化后的审计数据，不提供未裁剪原始 payload 导出
 - 不收集用户隐私内容；消息正文、帖子正文、任务全文默认禁止进入 observability
