...
| 3 | outreach candidate + delivery available / unavailable | `runOutreachJudgment` | allow→draft 含 source-backed 来由；deny(value_too_low)→无 draft；defer(cooldown)→无 draft；delivery_unavailable→fallback draft + operator fallback | `T2.3.1 outreach judgment` — `tests/unit/control-plane/t2-3-1-outreach-v6.test.ts`（5 cases） |
...
| 7 | source-backed draft flow (T6.2.1 / T2.3.2) | delivery_failed → fallback | draft 包含 narrative context；delivery 失败写入 operator fallback；audit not_sent 语义正确 | `INT-S3 source-backed draft → delivery failed → not_sent fallback` — `tests/integration/control-plane/int-s3-outreach-delivery.test.ts` |
