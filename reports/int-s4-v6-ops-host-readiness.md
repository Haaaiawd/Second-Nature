...
| 7 | carrier-only mode（无 workspaceRoot） | `heartbeat_check` | 返回 `runtimeMode:host_safe_carrier`；无 heartbeat_ok claim；无 lived-experience 断言 | `T1.1.3 heartbeatCheck runtime unavailable` — `tests/unit/cli/heartbeat-check.test.ts` |
