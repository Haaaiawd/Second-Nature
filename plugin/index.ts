...
interface SetupAckState {
  status: "pending" | "acknowledged" | "workspace_root_unknown" | "incomplete";
  markerPath?: string;
  acknowledgedAt?: string;
  placedIn?: string;
  incompleteReasons?: Array<{ field: string; reason: string; repairAction: string }>;
}
...
function validateSetupAck(raw: Record<string, unknown>): {
  ok: true;
} | {
  ok: false;
  errors: Array<{ field: string; reason: string; repairAction: string }>;
} {
  const errors: Array<{ field: string; reason: string; repairAction: string }> = [];
  ...
  const placementProofRef =
    typeof raw.placementProofRef === "string" ? raw.placementProofRef : undefined;
  ...
  const acknowledgedAt =
    typeof raw.acknowledgedAt === "string" ? raw.acknowledgedAt : undefined;
...
function buildHostDiscoveryReport(input?: Record<string, unknown>): {
  toolDiscovery: {
    status: "unsupported";
    tools: string[];
    hostName?: string;
  ...
  return {
    ...
    reason: "host_probe_unsupported",
    nextStep:
      "host_safe_carrier_cannot_probe_host_registry_run_workspace_cli_for_full_discovery",
  };
}
...
function readSetupAckMarker(spine: ActivationSpine): SetupAckState {
  ...
  try {
    ...
    const validation = validateSetupAck(marker as Record<string, unknown>);
    if (!validation.ok) {
      return {
        status: "incomplete",
        markerPath,
...
function buildSetupAckPayload(
  ...
  return {
    ...
    message:
      "Setup guide acknowledgement persisted; setup nudge is now silent for this workspace. Host skill discovery is unavailable in carrier mode.",
    data: {
      markerPath,
      hostDiscovery,
      ...marker,
...
function createHostSafeRouter(spine: ActivationSpine): CommandRouter {
  const commands: CommandDefinition[] = [
    ...
    },
    {
      ...
      execute: async () =>
        createUnavailableActionError(
          "HOST_SAFE_CONNECTOR_BEHAVIOR_ADD_UNAVAILABLE",
          "Connector behavior authoring writes workspace manifests; host-safe plugin cannot mutate connector files.",
          ["platformId", "behaviorId"],
          "run_workspace_second_nature_cli_or_full_runtime_package",
        ),
    },
    {
    ...
    },
    {
      ...
      execute: async () =>
        createUnavailableActionError(
          "HOST_SAFE_LOOP_STATUS_UNAVAILABLE",
          "loop_status requires workspace state database; provide workspaceRoot so the full workspace bridge can read persisted v8 loop state.",
          ["workspaceRoot"],
          "reinvoke_with_workspaceRoot_or_set_SECOND_NATURE_WORKSPACE_ROOT",
        ),
    },
  ];
