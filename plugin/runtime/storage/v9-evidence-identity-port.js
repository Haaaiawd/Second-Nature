...
function stableIdentityKey(item) {
    return item.externalId ?? item.contentHash ?? EMPTY_CONTENT_HASH;
}
function isUnstableItem(item) {
...
async function findEvidenceRow(db, item) {
...
function resolveRepetition(item, row) {
    ...
    if (item.contentHash && item.contentHash !== row.contentHash)
        return "changed";
    return "duplicate";
}
function buildStableIdentity(item, row) {
...
export const createEvidenceIdentityPort = (db) => ({
    async normalizeEvidenceIdentity(item) {
        if (isUnstableItem(item)) {
