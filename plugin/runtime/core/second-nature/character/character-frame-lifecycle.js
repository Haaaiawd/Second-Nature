...
function makeSourceRef(family, id) {
    return { family, id };
}
function hasFirstInjectionMarker(frame) {
    try {
...
function markFirstSeenPayload(now) {
    return JSON.stringify({ firstInjectedAt: now });
}
function sortBySourceCountAndConfidence(items) {
...
function sortBySourceCount(items) {
...
function buildFrameSummary(frame) {
    ...
    if (countUtf8Bytes(joined) <= POINTER_SUMMARY_MAX_CHARS)
        return joined;
    return truncateToBytes(joined, POINTER_SUMMARY_MAX_CHARS);
}
function serializePostureSourceRefs(refs) {
    if (refs.length === 0)
        return "";
...
function serializeValuePosture(v) {
...
function serializeGrowthTension(t) {
    return `- ${t.tension}${serializePostureSourceRefs(t.sourceRefs)}`;
}
function serializeFrameText(frame) {
    const sections = [];
    if (frame.emergentHabits && frame.emergentHabits.length > 0) {
...
function copyFrameForRevision(frame, now) {
    return {
        ...frame,
        id: randomUUID(),
...
export async function applyCharacterContest(frameId, action, store, options = {}) {
...
export async function supersedeFrame(previousId, newFrameId, store, options = {}) {
...
// ───────────────────────────────────────────────────────────────
export function buildCharacterFramePointer(frame, opts = {}) {
    return {
        frameId: frame.id,
...
export function buildEmbodiedContextProjection(frame, opts = {}) {
...
export function buildCharacterProjectionPair(frame, opts = {}) {
    return {
        pointer: buildCharacterFramePointer(frame, opts),
        projection: buildEmbodiedContextProjection(frame, opts),
    };
}
export async function loadActiveCharacterFrame(store, options = {}) {
...
export async function markFirstInjectionSeen(store, frameId, now = new Date().toISOString()) {
    const frame = await store.readFrameById(frameId);
    if (!frame)
        return;
