...
function makeBaseCandidate(runId, reviewId, family, suffix, text, sourceRefs, confidence) {
    return {
...
function generateMemoryCandidates(runId, reviewId, reviewPayload, baseRefs) {
    const candidates = [];
...
function generateProceduralCandidates(runId, reviewId, reviewPayload, baseRefs) {
    const candidates = [];
    const routineSignals = reviewPayload.routineSignals;
    if (routineSignals && routineSignals.length > 0) {
        for (let i = 0; i < routineSignals.length; i++) {
            const sig = routineSignals[i];
            if (!sig?.capabilityPattern)
                continue;
            const { text, blocked, reason } = redactSensitive(sig.summary);
            candidates.push({
                ...makeBaseCandidate(runId, reviewId, "procedural", `routine_${i}`, blocked ? text : `Routine candidate for ${sig.capabilityPattern}: ${text}`, baseRefs, blocked ? 0.3 : 0.6),
...
function generateSelfContinuityCandidates(runId, reviewId, reviewPayload, baseRefs) {
    const candidates = [];
    const continuitySignals = reviewPayload.continuitySignals;
    if (continuitySignals && continuitySignals.length > 0) {
        const { text, blocked, reason } = redactSensitive(continuitySignals.join("; "));
        candidates.push({
            ...makeBaseCandidate(runId, reviewId, "self_continuity", "refresh", blocked ? text : `Continuity refresh: ${text}`, baseRefs, blocked ? 0.3 : 0.5),
            validationStatus: blocked ? "blocked" : "valid",
            validationReason: blocked ? reason : undefined,
        });
    }
...
function generateConnectorEvolutionCandidates(runId, reviewId, reviewPayload, baseRefs) {
    const candidates = [];
    const evolutionSignals = reviewPayload.connectorEvolutionSignals;
    if (evolutionSignals && evolutionSignals.length > 0) {
        for (let i = 0; i < evolutionSignals.length; i++) {
            const sig = evolutionSignals[i];
            if (!sig?.platformId || !sig?.planType)
                continue;
            const { text, blocked, reason } = redactSensitive(sig.summary);
            candidates.push({
                ...makeBaseCandidate(runId, reviewId, "connector_evolution", `evolution_${i}`, blocked ? text : `Connector evolution for ${sig.platformId}: ${text}`, baseRefs, blocked ? 0.3 : 0.5),
                validationStatus: blocked ? "blocked" : "valid",
                validationReason: blocked ? reason : undefined,
...
function generateCharacterCandidates(runId, reviewId, reviewPayload, baseRefs) {
    const candidates = [];
    const characterSignals = reviewPayload.characterSignals;
    if (characterSignals && characterSignals.length > 0) {
        const { text, blocked, reason } = redactSensitive(characterSignals.join("; "));
        candidates.push({
            ...makeBaseCandidate(runId, reviewId, "character", "refresh", blocked ? text : `Character refresh: ${text}`, baseRefs, blocked ? 0.3 : 0.4),
            validationStatus: blocked ? "blocked" : "valid",
            validationReason: blocked ? reason : undefined,
        });
    }
...
function generateCandidates(runId, reviewId, reviewPayload, baseRefs) {
    return [
...
// Public API
// ───────────────────────────────────────────────────────────────
export async function runV9DreamConsolidation(db, runId, options) {
    ...
    const validFamilies = Array.from(new Set(candidates
