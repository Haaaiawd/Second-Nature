...
const FORBIDDEN_RULES = [
    {
        ...
        patterns: [
            /\byou\s+feel\s+/gi,
            /\byour\s+true\s+emotion\s+is\b/gi,
            /\b你\s*(?:感到|觉得)\s*/g,
            /\b你的真实情绪是\b/g,
        ],
    },
    {
        id: "identity_lock",
        patterns: [
            /\byou\s+are\s+a\s+(?:loyal|kind|selfless|anxious|controlling|toxic|people[-\s]?pleasing)\s+person\b/gi,
            /\byou\s+are\s+the\s+kind\s+of\s+person\s+who\s+never\s+changes\b/gi,
            /你就是这样的人/g,
            /你是(?:讨好型人格|忠诚的人|不会改变的人)/g,
        ],
    },
    {
        id: "hard_control",
        patterns: [
            /\byou\s+must\s+always\s+/gi,
            /\bnever\s+disagree\b/gi,
            /\bnever\s+question\b/gi,
            /\b你必须\s*(?:永远|始终|一直)/g,
            /\b永远不要质疑\b/g,
            /\b永远不要不同意\b/g,
        ],
