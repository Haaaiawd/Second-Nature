...
function toSourceRef(family, id, label) {
    if (!id)
        return undefined;
    return { family, id, label };
}
function sourceRefOrEmpty(family, id) {
    const ref = toSourceRef(family, id);
    return ref ? [ref] : [];
}
function collectSourceRefs(probe, experiences, routineId) {
    const refs = [];
    if (probe) {
...
function deriveAccessLevel(registered, hasCredential) {
    if (!registered)
        return "none";
    if (!hasCredential)
        return "needs_auth";
...
function deriveReliabilityLevel(probe, experiences, nowMs) {
...
function deriveFamiliarityLevel(experiences, routineId, registered) {
    if (!registered)
        return "scaffold";
    if (routineId)
        return "routine";
    const successCount = experiences.filter((e) => e.outcome === "success").length;
    if (successCount >= FAMILIARITY_SUCCESS_THRESHOLD)
        return "practiced";
    return "scaffold";
}
function lastExecutedAt(experiences) {
    return experiences[0]?.createdAt;
}
export async function assembleToolAffordance(deps, query = {}) {
    ...
    const filtered = allCapabilities.filter((cap) => {
        if (query.platformId && cap.platformId !== query.platformId)
            return false;
        if (query.capabilityId && cap.capabilityId !== query.capabilityId)
            return false;
        return true;
    });
    const postures = [];
    for (const cap of filtered) {
        ...
        const familiarityLevel = deriveFamiliarityLevel(experiences, routine?.id ?? undefined, registered);
...
export function createStaticRegistry(capabilities) {
    return {
...
export function createCredentialPresenceFromVault(vault) {
    return {
        async hasCredential(platformId) {
