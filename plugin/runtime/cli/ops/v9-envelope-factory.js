 ...
 */
import { containsCredentialValue, } from "../../observability/v9-redaction-projector.js";
// ───────────────────────────────────────────────────────────────
...
export function classifySensitiveField(key) {
    if (CREDENTIAL_KEY_PATTERNS.some((p) => p.test(key)))
        return "credential";
    if (PRIVATE_CONTENT_KEY_PATTERNS.some((p) => p.test(key)))
        return "private";
    if (PROMPT_KEY_PATTERNS.some((p) => p.test(key)))
        return "prompt";
    return "none";
}
// ───────────────────────────────────────────────────────────────
 ...
 */
export function redactOpsPayload(payload, depth = 0) {
    const collector = createDiagnosticsCollector();
    const redacted = redactRecursive(payload, "", collector, depth);
    return { redacted, diagnostics: collector.result() };
}
function redactRecursive(value, keyPath, collector, depth) {
    if (depth > 10)
        return value; // depth limit
 ...
 */
export function capEvidenceLevel(level, surfaceMode) {
 ...
 */
export function promoteEvidence(current, signals) {
    let level = current;
    if (signals.hasSourceRefs)
        level = maxLevel(level, "state_present");
    if (signals.hasRealRuntimeProof)
        level = maxLevel(level, "real_runtime");
    if (signals.hasDurableAudit)
...
// ───────────────────────────────────────────────────────────────
export function redactOpsPayloadBatch(payloads) {
