 ...
 */
export async function rollbackHealthGate(deps, plan) {
    const now = deps.now();
    const events = await deps.listStageEvents(plan.createdAt, now.toISOString());
    // Filter rollback events matching this plan.id
    const rollbackEvents = events.filter((e) => e.stageKind === "rollback" && (e.traceRefsJson ?? "").includes(plan.id));
    ...
    const failure = rollbackEvents.some((e) => e.status === "blocked" || e.reasonCode === "evolution_rollback_failed");
    if (failure) {
        return {
            status: "blocked",
 ...
 */
export async function rollbackHealthGateBatch(deps, plans) {
    const results = [];
 ...
 */
export function needsWatchdogMonitoring(plan) {
