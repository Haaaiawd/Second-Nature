...
import type { LifeEvidence, LifeEvidenceCandidate, LifeEvidenceWriteAck } from "./types.js";

export interface AppendLifeEvidenceOptions {
  provenance?: ProvenanceRepository;
}

export async function appendLifeEvidence(
  state: StateDatabase,
  workspaceRoot: string,
  candidate: LifeEvidenceCandidate,
  options?: AppendLifeEvidenceOptions
): Promise<LifeEvidenceWriteAck> {
  ...
  const artifactRef: LifeEvidence["artifactRef"] = {
    id: `artifact:${evidenceId}`,
    kind: "workspace_artifact",
    uri: artifactRel.replace(/\\/g, "/"),
  };

  const life: LifeEvidence = {
    id: evidenceId,
  ...
  if (options?.provenance) {
    await options.provenance.linkEntrySources(
      evidenceId,
      candidate.sourceRefs.map((ref) => ref.id)
    );
  }
