...
function applyStateSchemaMigrations(sqlite: Database): void {
  ...
  const addColumnMigrations = [
    ...
    "ALTER TABLE character_frame ADD COLUMN valid_until TEXT",
    "ALTER TABLE character_frame ADD COLUMN valid_from TEXT",
    "ALTER TABLE character_frame ADD COLUMN char_count INTEGER NOT NULL DEFAULT 0",
  ...
  for (const table of dropColumnTables) {
    try {
      if (supportsDropColumn) {
        ...
        if (kept.length === allNames.length) continue; // column already absent
