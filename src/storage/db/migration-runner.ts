...
function ensureMetaTable(sqlite: Database): void {
...
function markMigrationFailed(
  ...
  sqlite.exec(
    "INSERT OR REPLACE INTO _meta (key, value) VALUES ('schema_migration_failed', '1')"
  );
  const safeError = error.replace(/'/g, "''");
  sqlite.exec(
    `INSERT OR REPLACE INTO _meta (key, value) VALUES ('schema_migration_failed_version', '${version}')`
  );
  sqlite.exec(
    `INSERT OR REPLACE INTO _meta (key, value) VALUES ('schema_migration_failed_error', '${safeError}')`
  );
}

export function isMigrationFailed(sqlite: Database): boolean {
  const rows = sqlite.exec(
    "SELECT value FROM _meta WHERE key = 'schema_migration_failed'"
  );
  return (
    rows.length > 0 &&
  ...
  for (const migration of pending) {
    try {
      ...
      process.stderr.write(
        `schema_migration_failed: v${migration.version} (${migration.label}) — ${message}\n`
      );
