...
async function listCuratedSignalFiles(workspaceRoot: string): Promise<string[]> {
...
export async function loadUserInterestSnapshot(workspaceRoot: string): Promise<UserInterestSnapshot> {
  ...
  const curatedNonEmpty: string[] = [];
  for (const fp of curatedFiles) {
    const body = (await readText(fp)).trim();
    if (body.length >= MIN_MEANINGFUL_ANCHOR_CHARS) curatedNonEmpty.push(fp);
  }
  ...
  const mtimes = await newestMtime(
    [hasUser ? userPath : "", hasMemory ? memoryPath : "", ...curatedNonEmpty].filter(Boolean),
  );
  const staleness: UserInterestStaleness =
    mtimes > 0 && Date.now() - mtimes > STALE_MS ? "stale" : "fresh";
