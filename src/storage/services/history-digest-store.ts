...
export function createHistoryDigestStore(
  ...
  return {
    async appendNarrativeTimeline(entry) {
      ...
      if (!deltaGate.ok) throw new Error(deltaGate.reason ?? "write_validation_failed");
    ...
    async listNarrativeTimeline(query = {}) {
      ...
      return result[0]!.values.map((row) => ({
        ...
        delta: safeParseJson<Record<string, unknown>>(
          (get(row, "delta_json") as string) ?? "{}",
          {},
        ),
        previousHash: (get(row, "previous_hash") as string) ?? "",
        currentHash: (get(row, "current_hash") as string) ?? "",
    ...
    async loadHeartbeatDigest(day) {
      const result = sqlite.exec(
        `SELECT * FROM heartbeat_digest WHERE day = ?`,
        [day],
      );
      if (result.length === 0 || result[0]!.values.length === 0) {
        return undefined;
      ...
      const cols = result[0]!.columns;
      const get = (row: unknown[], name: string) =>
        row[cols.indexOf(name)] as string | number | null;
      const row = result[0]!.values[0]!;

      return {
        digestId: get(row, "digest_id")! as string,
        day: get(row, "day")! as string,
        connectorSummary: safeParseJson<
          HeartbeatDigest["connectorSummary"]
        >((get(row, "connector_summary_json") as string) ?? "[]", []),
        goalSummary: safeParseJson<
          HeartbeatDigest["goalSummary"]
        >((get(row, "goal_summary_json") as string) ?? "[]", []),
        quietCount: (get(row, "quiet_count") as number) ?? 0,
        dreamCount: (get(row, "dream_count") as number) ?? 0,
        breakerSummary: safeParseJson<
          HeartbeatDigest["breakerSummary"]
        >((get(row, "breaker_summary_json") as string) ?? "[]", []),
        healthStatus: (get(row, "health_status") as string) ?? "ok",
