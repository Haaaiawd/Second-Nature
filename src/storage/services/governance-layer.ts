...
export class GovernanceLayer {
  ...
  async proposeAnchorWrite(proposal: AnchorWriteProposal): Promise<ProposalAck> {
  ...
  async applyGovernedAnchorWrite(proposalId: string): Promise<ApplyAck> {
  ...
  async loadProposal(proposalId: string): Promise<AnchorWriteProposal | null> {
    ...
    return {
      ...
      status: record.status as AnchorWriteProposal["status"],
