...
export interface DiaryDreamStore {
  ...
  transitionDreamOutputLifecycle(
    outputId: string,
    newStatus: Exclude<DreamOutputStatus, "candidate" | "partial">,
  ): Promise<void>;
...
export function createDiaryDreamStore(database: StateDatabase): DiaryDreamStore {
  ...
  return {
    ...
    async loadDailyDiary(day) {
      const result = sqlite.exec(
        `SELECT * FROM daily_diary_index WHERE day = ?`,
        [day],
      );
      ...
      return {
        ...
        day: get(row, "day")!,
        observedToday: safeParseJson<string[]>(
          (get(row, "observed_today_json") as string) ?? "[]",
          [],
        ),
        notableSignals: safeParseJson<string[]>(
          (get(row, "notable_signals_json") as string) ?? "[]",
          [],
        ),
        tomorrowDirection: (get(row, "tomorrow_direction") as string) ?? "",
        sourceRefs: safeParseJson<readonly [string, ...string[]]>(
          (get(row, "source_refs_json") as string) ?? "[]",
          ["store:default"],
        ),
    ...
    async transitionDreamOutputLifecycle(outputId, newStatus) {
      const currentResult = sqlite.exec(
        `SELECT status FROM dream_output_index WHERE output_id = ?`,
        [outputId],
      );
      if (
      ...
      const currentStatus = currentResult[0]!.values[0]![0] as DreamOutputStatus;
      ...
      sqlite.run(
        `UPDATE dream_output_index SET status = ? WHERE output_id = ?`,
        [newStatus, outputId],
      );
    },
    ...
    async loadAcceptedDreamProjection(limit = 10) {
      const result = sqlite.exec(
        `SELECT * FROM dream_output_index
...
function rowToDreamOutput(
  ...
  return {
    ...
    runId: get("run_id")!,
    status: get("status")! as DreamOutputStatus,
    canonicalEntries: safeParseJson<unknown[]>(
      (get("canonical_entries_json") as string) ?? "[]",
      [],
    ),
    insights: safeParseJson<unknown[]>(
      (get("insights_json") as string) ?? "[]",
      [],
    ),
    narrativeUpdate: safeParseJson<Record<string, unknown> | undefined>(
      (get("narrative_update_json") as string) ?? "null",
      undefined,
    ),
    relationshipUpdate: safeParseJson<Record<string, unknown> | undefined>(
      (get("relationship_update_json") as string) ?? "null",
      undefined,
    ),
    validation: safeParseJson<
      DreamOutput["validation"]
    >((get("validation_json") as string) ?? "{}", {
      schemaValid: false,
