...
import type { RuntimeSecretAnchor, SecretAnchorHealth } from "../../shared/types/v7-entities.js";
...
export function createRuntimeSecretAnchorStore(
  database: StateDatabase,
): RuntimeSecretAnchorStore {
  ...
  return {
    async upsertAnchor(anchor) {
      ...
      if (!gate.ok) {
        throw new Error(
          `RuntimeSecretAnchor rejected by gate: ${gate.reason}`,
        );
      }

      sqlite.run(
        `INSERT INTO runtime_secret_anchor
    ...
    async loadAnchor(anchorId) {
      const result = sqlite.exec(
        `SELECT * FROM runtime_secret_anchor WHERE anchor_id = ?`,
        [anchorId],
      );
      ...
      return {
        ...
        health: get(row, "health")! as SecretAnchorHealth,
        rotationPolicyRef: (get(row, "rotation_policy_ref") as string | null) ?? undefined,
    ...
    async listAnchors() {
      const result = sqlite.exec(
        `SELECT * FROM runtime_secret_anchor ORDER BY updated_at DESC`,
      );
      if (result.length === 0 || result[0]!.values.length === 0) {
        return [];
      }
