...
export class RepairAndBackupService {
  ...
  async runStartupRepair(options: RepairAndBackupOptions = {}): Promise<RepairAndBackupResult> {
    ...
    const repairedOrphanAssetIds: string[] = [];
    const updatedHashAssetIds: string[] = [];
    ...
    const staleProposals = await this.database.db
      ...
      .where(
        and(
          inArray(proposalRecords.status, ["draft", "requires_review"]),
          lt(proposalRecords.createdAt, staleCutoff)
        )
      );
...
export function createRepairAndBackupService(database: StateDatabase): RepairAndBackupService {
