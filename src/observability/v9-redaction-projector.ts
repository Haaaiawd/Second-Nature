 ...
 */
export function containsCredentialValue(payloadJson: string): boolean {
  if (!payloadJson || payloadJson.trim() === "") return false;

  // Check for credential-shaped values in the raw string.
  for (const pattern of CREDENTIAL_VALUE_PATTERNS) {
    if (pattern.test(payloadJson)) return true;
  }
...
function scanObjectForCredentialValues(obj: unknown, depth = 0): boolean {
  if (depth > 10) return false; // depth limit to prevent stack overflow
  if (obj === null || obj === undefined) return false;
  if (typeof obj === "string") {
    for (const pattern of CREDENTIAL_VALUE_PATTERNS) {
      if (pattern.test(obj)) return true;
    }
    return false;
  }
  ...
  for (const [, value] of Object.entries(record)) {
    if (scanObjectForCredentialValues(value, depth + 1)) return true;
  }
  return false;
}

// ───────────────────────────────────────────────────────────────
// Payload redaction result
// ───────────────────────────────────────────────────────────────
...
export function redactPayloadJson(
  ...
  if (!payloadJson || payloadJson.trim() === "") return empty;
  ...
  const result: RedactPayloadResult<Record<string, unknown>> = redactPayload(
    parsed as Record<string, unknown>,
    policy,
  );

  const wasRedacted =
    result.manifest.maskedPaths.length > 0 ||
 ...
 */
export function redactLedgerEntry(
  redactedPayloadJson: string | undefined | null,
  policy: RedactionPolicy = DEFAULT_REDACTION_POLICY,
): LedgerRedactionResult {
...
export function redactTimelinePayload(
  ...
  const hasCredential = containsCredentialValue(payloadJson);
  if (hasCredential) {
    // Replace entire payload with a safe placeholder — credential values
    // in non-sensitive fields would survive key-based redaction alone.
    return {
      json: JSON.stringify({ redacted: "credential_value_detected" }),
      containsCredentialValue: true,
      wasRedacted: true,
      manifest: { maskedPaths: [], erasedPaths: ["$"], hashedPaths: [] },
    };
  ...
  // No credential values — apply normal key-based redaction.
 ...
 */
export function redactCharacterFrameEvent(
  payloadJson: string | undefined | null,
  policy: RedactionPolicy = DEFAULT_REDACTION_POLICY,
): LedgerRedactionResult {
 ...
 */
export function validateCharacterSafety(text: string): CharacterSafetyResult {
