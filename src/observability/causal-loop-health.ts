...
function maxCycleSequence(stages: StageHealth[]): number {
  return Math.max(0, ...stages.map((s) => s.lastCycleSequence ?? 0));
}

function findStalledStage(
  stages: StageHealth[],
  threshold: number,
): LoopStage | undefined {
  ...
  for (const stage of stages) {
    if (stage.lastCycleSequence === undefined) {
      ...
      if (maxSeq > 0) return stage.stage;
      continue;
    }
...
export async function assembleLoopStatus(
  db: StateDatabase,
  options?: AssembleLoopStatusOptions,
): Promise<CausalLoopHealthSnapshot | DegradedOperationResult> {
  ...
  const overallStatus: CausalLoopHealthSnapshot["overallStatus"] = stalledAt
    ? "stalled"
    : "healthy";
