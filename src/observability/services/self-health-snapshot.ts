...
const PROBE_TIMEOUTS: Record<string, number> = {
  env: 200,
  storage: 200,
  cron: 500,
  bridge: 500,
...
export const MINIMUM_REQUIRED_DIMENSIONS: string[] = [
  "env",
  "cron",
  "secret",
  "credential",
 ...
 */
export function registerHealthProbe(probe: RegisteredProbe): void {
 ...
 */
export function unregisterHealthProbe(dimensionId: string): void {
 ...
 */
export function clearHealthProbeRegistry(): void {
  _probeRegistry.clear();
}

/** Get all currently registered probes. */
export function getRegisteredProbes(): RegisteredProbe[] {
  return Array.from(_probeRegistry.values());
}

// ─── Default probes for minimum dimensions ───────────────────────────────────

function makeDefaultProbe(dimensionId: string): RegisteredProbe {
 ...
 */
export function ensureMinimumProbes(): void {
...
async function runProbeWithTimeout(
  rp: RegisteredProbe,
  now: string,
): Promise<[string, DimensionHealth]> {
  const timeoutStatus: HealthStatus = rp.timeoutAsDegraded ? "degraded" : "unknown";
  ...
  const timeoutPromise = new Promise<DimensionHealth>((resolve) => {
    setTimeout(() => {
      resolve({
  ...
  const probePromise = rp.probe().catch((err: unknown): DimensionHealth => ({
    status: "unknown",
 ...
 */
export async function getSelfHealthSnapshot(
  ...
  const globalTimeoutPromise = new Promise<
    Array<[string, DimensionHealth]>
  >((resolve) => {
    setTimeout(() => {
      // Return all probes as unknown on global timeout
      const fallback = probes.map((p): [string, DimensionHealth] => [
        p.dimensionId,
        {
  ...
  const probeRunnerPromise = Promise.allSettled(
    probes.map((p) => runProbeWithTimeout(p, now)),
  ).then((results) =>
    results
      .filter((r) => r.status === "fulfilled")
      .map((r) => (r as PromiseFulfilledResult<[string, DimensionHealth]>).value),
  );
  ...
  for (const [dimId, health] of Object.entries(dimensions)) {
    if (health.status !== "healthy") {
      ...
      if (health.reason) diagnosticReasonCodes.push(health.reason);
    }
  }
