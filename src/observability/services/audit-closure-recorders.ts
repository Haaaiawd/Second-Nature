...
export type ConnectorAttemptAuditOutcome =
  | "success"
  | "failure"
  | "circuit_open"
  | "blocked";
...
function mapConnectorOutcome(result: ConnectorResult<unknown>): ConnectorAttemptAuditOutcome {
  if (result.status === "success") return "success";
  if (result.failureClass === "cooldown_blocked") return "blocked";
  if (result.failureClass === "platform_unavailable") return "circuit_open";
  return "failure";
}

export function recordConnectorAttemptAudit(input: RecordConnectorAttemptAuditInput): { eventId?: string } {
...
export function recordQuietArtifactAudit(input: RecordQuietArtifactAuditInput): { eventId?: string } {
  if (!input.auditStore) return {};
  const createdAt = input.createdAt ?? new Date().toISOString();
