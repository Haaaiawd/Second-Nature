...
export interface ReflectionPorts {
  loadQuietInputs(query: { lookbackDays: number }): Promise<{ sourceRefs: string[] }>;
  generateNarrativeReflection(input: { sourceRefs: string[] }): Promise<ReflectionGeneration>;
  filterAllowedWrites(writes: ProposedWrite[]): ProposedWrite[];
}

export async function runNarrativeReflection(
  ports: ReflectionPorts,
