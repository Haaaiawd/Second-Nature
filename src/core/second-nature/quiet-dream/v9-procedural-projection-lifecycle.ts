...
import {
  ...
  updateProceduralProjectionStatus,
...
function generateProjectionId(candidateId: string): string {
  return `proc_${candidateId}_${randomUUID()}`;
}

function toV8SourceRefs(refs: SourceRef[]): import("../../../shared/types/v8-contracts.js").SourceRef[] {
  return refs as unknown as import("../../../shared/types/v8-contracts.js").SourceRef[];
}
...
export async function acceptProceduralProjection(
  db: StateDatabase,
  candidateId: string,
  capabilityPattern: string,
  routineText: string,
  sourceRefs: SourceRef[],
  options?: AcceptProceduralProjectionOptions,
): Promise<ProceduralProjectionLifecycleResult | DegradedOperationResult> {
  ...
  let supersedesProjectionId: string | undefined;
...
export async function rejectProceduralProjection(
  db: StateDatabase,
  projectionId: string,
): Promise<ProceduralProjectionLifecycleResult | DegradedOperationResult> {
...
export async function retireProceduralProjection(
  db: StateDatabase,
  projectionId: string,
): Promise<ProceduralProjectionLifecycleResult | DegradedOperationResult> {
