...
} from "./character-refresh-input-normalizer.js";
...
export type CharacterFrameResult =
  | AcceptedCharacterFrameResult
  | CharacterFrameDeferredResult;
...
function countUtf8Chars(text: string): number {
  return new TextEncoder().encode(text).length;
}

function selectLocalePrompt(locale: CharacterRefreshInput["locale"]): string {
  if (locale === "zh-CN") return CONTEST_PROMPT_ZH;
  if (locale === "en") return CONTEST_PROMPT_EN;
  return `${CONTEST_PROMPT_ZH} / ${CONTEST_PROMPT_EN}`;
}

function groupBySignalKind(
  signals: CharacterSignal[],
): Partial<Record<CharacterSignalKind, CharacterSignal[]>> {
  return signals.reduce((acc, signal) => {
    (acc[signal.signalKind] ??= []).push(signal);
    return acc;
  }, {} as Partial<Record<CharacterSignalKind, CharacterSignal[]>>);
}

function confidenceFromSignals(signals: CharacterSignal[]): EmergentHabit["confidence"] {
  if (signals.length === 0) return "low";
  const highs = signals.filter((s) => s.confidence === "high").length;
  const mediums = signals.filter((s) => s.confidence === "medium").length;
  if (highs > mediums) return "high";
  if (mediums >= 1) return "medium";
  return "low";
}

function extractHabits(signals: CharacterSignal[]): EmergentHabit[] {
  const byKind = groupBySignalKind(signals);
  const habits: EmergentHabit[] = [];
  ...
  if (toolExperiences.length > 0) {
    ...
    if (pattern) {
      habits.push({
        ...
        sourceRefs: deduplicateSourceRefs(
          toolExperiences.flatMap((s) => s.sourceRefs),
        ),
  ...
  if (closures.length >= 2) {
    habits.push({
      description: truncateToChars(
        `重复出现的行动闭环: ${summarizeSharedTheme(closures.map((s) => s.summary))}`,
        120,
      ),
  ...
  if (dreams.length > 0) {
    habits.push({
      description: truncateToChars(
        `Dream 压缩出的习惯方向: ${summariesToShortPhrase(dreams.map((s) => s.summary))}`,
        120,
      ),
...
function extractValuePosture(signals: CharacterSignal[]): ValuePosture | null {
  const feedback = signals.filter(
    (s) => s.signalKind === "owner_feedback" || s.signalKind === "relationship_signal",
  );
...
function extractRelationshipPosture(
  signals: CharacterSignal[],
): RelationshipPosture | null {
  const feedback = signals.filter(
    (s) =>
      s.signalKind === "relationship_signal" ||
...
function extractExpressionPosture(
  signals: CharacterSignal[],
...
function extractGrowthTensions(signals: CharacterSignal[]): GrowthTension[] {
  const feedback = signals.filter(
    (s) => s.signalKind === "owner_feedback" || s.signalKind === "relationship_signal",
  );
  const tensions: GrowthTension[] = [];

  const hasWriteSignal = signals.some((s) =>
    /external write|publish|reply/i.test(s.summary),
  );
  const hasCautionSignal = feedback.some((s) =>
    /cautious|slow|hesitant|uncertain/i.test(s.summary),
  );
...
function detectConflictNotes(signals: CharacterSignal[]): ConflictNote[] {
  const notes: ConflictNote[] = [];
  const feedback = signals.filter(
    (s) => s.signalKind === "owner_feedback" || s.signalKind === "relationship_signal",
  );
  if (feedback.length < 2) return notes;
  ...
  if (positive.length > 0 && negative.length > 0) {
    notes.push({
      ...
      conflictingSourceRefs: deduplicateSourceRefs(
        [...positive, ...negative].flatMap((s) => s.sourceRefs),
      ).slice(0, 10),
    });
  }
...
function collectShortLabels(summaries: string[]): string[] {
...
function summarizeRecurringPattern(summaries: string[]): string | null {
  if (summaries.length === 0) return null;
  const shared = summariesToShortPhrase(summaries);
  return `从工具经历中观察到重复模式: ${shared}`;
}

function summarizeSharedTheme(summaries: string[]): string {
  return summariesToShortPhrase(summaries);
}

function summariesToShortPhrase(summaries: string[]): string {
...
function buildSections(input: CharacterRefreshInput) {
...
function charCountOfTextualParts(frame: CharacterFrame): number {
  const parts = [
...
async function assignVersion(store: CharacterFrameStorePort): Promise<number> {
...
export async function refreshCharacterFrame(
  input: CharacterRefreshInput,
  store: CharacterFrameStorePort,
  options: BuildCharacterFrameOptions = {},
  ...
  const contestPrompt = truncateToChars(
    selectLocalePrompt(input.locale),
    CONTEST_PROMPT_MAX_CHARS,
  );
  ...
  while (candidate.charCount > CHARACTER_FRAME_MAX_CHARS && iterations < 5) {
    ...
    if (candidate.expressionPosture) {
      candidate.expressionPosture.styleNotes = candidate.expressionPosture.styleNotes.map((note) =>
        truncateToBytes(note, Math.max(30, countUtf8Chars(note) * 0.8)),
      );
      candidate.expressionPosture.boundaryConstraints = candidate.expressionPosture.boundaryConstraints?.map((c) =>
        truncateToBytes(c, Math.max(30, countUtf8Chars(c) * 0.8)),
      );
    }
