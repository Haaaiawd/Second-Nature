...
const CANONICAL_SECTION_ORDER: (keyof SelfContinuityCardSections)[] = [
  "summary",
  "bodyIntuition",
  "relationshipPosture",
  "valuePosture",
...
export function truncateToChars(text: string, maxChars: number): string {
  ...
  if (maxChars <= ellipsis.length) return chars.slice(0, maxChars).join("");
...
function toRoutinePointer(routine: ToolRoutine | RoutineListItem): RoutinePointer {
  return {
...
function toMemoryProjection(row: { id: string; sourceRefsJson: string }): MemoryProjection {
  return {
    id: row.id,
    kind: "memory",
...
function toProceduralProjection(row: { id: string; sourceRefsJson: string }): ProceduralProjection {
  return {
    id: row.id,
    kind: "procedural",
    sourceRefs: parseSourceRefs(row.sourceRefsJson),
...
function makeDegraded(reason: string, operatorNextAction: string): DegradedOperationResult {
  return {
    status: "unavailable",
    reason: reason as DegradedOperationResult["reason"],
...
function toToolRoutine(row: ToolRoutineRecord): ToolRoutine {
  ...
  return {
    ...
    status: row.status as ToolRoutine["status"],
    sourceRefs: parseSourceRefs(row.sourceRefsJson),
...
function toRoutinePointerFromRecord(row: ToolRoutineRecord): RoutinePointer {
  return {
    routineId: row.id,
    capabilityPattern: row.capabilityPattern,
    version: row.version,
...
function buildSummary(memories: MemoryProjection[]): string {
  if (memories.length === 0) return "continuity summary: no active memory projections";
  return `continuity summary: ${memories.length} active memory projection(s)`;
}

function buildBodyIntuition(memories: MemoryProjection[], procedurals: ProceduralProjection[]): string {
  const parts: string[] = [];
  if (memories.length > 0) parts.push("memory patterns available");
  if (procedurals.length > 0) parts.push(`${procedurals.length} procedural routine pattern(s)`);
  if (parts.length === 0) return "body intuition: no learned patterns yet";
  return `body intuition: ${parts.join("; ")}`;
}

function buildRelationshipPosture(memories: MemoryProjection[]): string {
  if (memories.length === 0) return "relationship posture: observation-oriented, awaiting owner signals";
  return "relationship posture: responsive and observation-oriented";
}

function buildValuePosture(memories: MemoryProjection[]): string {
  if (memories.length === 0) return "value posture: continuity over assertion";
  return "value posture: source-backed, contestable";
}

function buildBehaviorHabits(routines: ToolRoutine[]): string[] {
  if (routines.length === 0) return ["behavior habits: no active routines"];
  return routines.slice(0, 5).map((r) =>
    truncateToChars(`${r.capabilityPattern} (${r.status})`, HABIT_MAX_CHARS)
  );
}

function buildCurrentProhibitions(): string[] {
  return [
...
function buildCardSections(
  ...
  procedurals: ProceduralProjection[],
  routines: ToolRoutine[],
  characterPointer: CharacterFramePointer,
): SelfContinuityCardSections {
  return {
...
function serializeCardSections(sections: SelfContinuityCardSections): string {
  const lines: string[] = [];
...
function applyCardBudget(
  sections: SelfContinuityCardSections,
  characterPointer: CharacterFramePointer,
  maxChars: number,
  maxBytes: number,
): { cardText: string; sections: SelfContinuityCardSections } {
  ...
  const trimmed: SelfContinuityCardSections = {
    ...sections,
...
export function createCharacterFrameStoreAdapter(db: StateDatabase): CharacterFrameStorePort {
  return {
    ...
    async updateFrameLifecycle(frameId: string, status: import("../../../shared/types/v9-contracts.js").CharacterFrameStatus, opts?: {
      supersededBy?: string;
...
export async function assembleSelfContinuityCard(
  db: StateDatabase,
  scope: ContinuityScope,
): Promise<AssembleSelfContinuityCardResult | DegradedOperationResult> {
  ...
  if (memoryRead.degraded) return memoryRead.degraded;
  if (proceduralRead.degraded) return proceduralRead.degraded;
  if (routineRead.degraded) return routineRead.degraded;
  ...
  const deferredPointer: CharacterFramePointer = {
    frameId: "deferred",
  ...
  const redactedSectionsJson = JSON.stringify(
    redactPayload(budgetedSections as unknown as Record<string, unknown>).payload,
  ) as unknown as SelfContinuityCardSections;
...
export function createContinuityReadPort(db: StateDatabase): ContinuityReadPort {
  return {
    async loadSelfContinuityCard(scope: ContinuityScope): Promise<SelfContinuityCard | DegradedOperationResult> {
      const latest = await readLatestSelfContinuityCard(db);
      if (latest.degraded) return latest.degraded;
      if (!latest.row || latest.row.status !== "active") {
        const assembled = await assembleSelfContinuityCard(db, scope);
        if ("card" in assembled) return assembled.card;
        return assembled;
      }
    ...
    async loadRoutineList(filters) {
      ...
      const registryStatuses: Array<ToolRoutine["status"]> = status.flatMap((s) => {
      ...
      for (const st of registryStatuses) {
        const { rows, degraded } = await readToolRoutinesByStatus(db, st as ToolRoutineRecord["status"]);
        if (degraded) return { routines: [], degraded };
      ...
      const routines: RoutineListItem[] = all.map((r) => ({
        routineId: r.id,
    ...
    async loadActiveMemoryProjections() {
      ...
      if (degraded) return { projections: [], degraded };
    ...
    async loadActiveCharacterFramePointer(scope: ContinuityScope) {
      const characterStore = await createCharacterFrameStoreAdapter(db);
...
function rowToCard(row: SelfContinuityCardRecord): SelfContinuityCard {
  const sections = JSON.parse(row.sectionsJson) as SelfContinuityCardSections;
  return {
    id: row.id,
    ...
    characterFramePointer: JSON.parse(row.characterFramePointerJson) as CharacterFramePointer,
    sourceRefs: parseSourceRefs(row.sourceRefsJson),
    acceptedAt: row.createdAt,
    status: row.status as SelfContinuityCard["status"],
    redactionClass: (row.redactionClass as SelfContinuityCard["redactionClass"]) ?? "none",
  };
}

function mapRoutineStatus(status: ToolRoutine["status"]): RoutineListItem["status"] {
