...
export type GuidanceValidationResult =
  | { ok: true }
  | { ok: false; reason: V8ReasonCode; sourceRefs: SourceRef[] };

export type ConsumeGuidanceProposalResult =
  | { ok: true; output: GuidanceOutput }
...
function validateSourceRefs(
  ...
  const unresolved = allRefs.filter(
    (r) => r.resolveStatus === "missing" || r.resolveStatus === "permission_denied",
  );
...
export function consumeGuidanceProposal(
  proposal: ActionProposal,
  decision: ActionPolicyDecision,
): ConsumeGuidanceProposalResult {
