...
export type AdvanceActivityThreadResult =
  | { status: "advanced"; thread: ActivityThread; step: ActivityStep }
  | { status: "stopped"; thread: ActivityThread; reason: string }
...
export function createActivityThreadCoordinator(
  deps: ActivityThreadCoordinatorDeps,
) {
  return {
    async advanceActivityThread(
      options: AdvanceActivityThreadOptions,
...
export async function advanceActivityThread(
  options: AdvanceActivityThreadOptions & ActivityThreadCoordinatorDeps,
): Promise<AdvanceActivityThreadResult> {
...
function stopReason(thread: ActivityThread, cycleSequence: number): string {
  if (thread.completedStepCount >= ACTIVITY_THREAD_MAX_STEPS) return "activity_thread_overlong";
...
function chooseNextStepKind(
  thread: ActivityThread,
  attention: AttentionSignal,
): ActivityStep["stepKind"] {
  if (attention.threadSuggestion === "complete") return "complete";
  if (attention.threadSuggestion === "pause") return "pause";
  if (attention.threadSuggestion === "none") return "observe";
  ...
  const rotation: ActivityStep["stepKind"][] = [
    "observe",
    "associate",
    "ask_agent",
    "propose_action",
  ...
  for (let offset = 1; offset <= rotation.length; offset++) {
    ...
    if (possible.includes(candidate)) return candidate;
  }
  return possible[0] ?? "observe";
}
...
function summarizeFocus(attention: AttentionSignal): string {
  return attention.summary.length <= 200 ? attention.summary : attention.summary.slice(0, 197) + "...";
}

function deriveAssociations(attention: AttentionSignal, context: EmbodiedContext): string[] {
...
function deriveNextPossibleMoves(attention: AttentionSignal): ActivityStep["stepKind"][] {
  ...
  const base: ActivityStep["stepKind"][] = ["associate"];
  for (const action of attention.possibleActions) {
    if (action === "notify_owner") base.push("propose_action");
    if (action === "watch") base.push("observe");
    if (action === "remember") base.push("associate");
    if (action === "defer") base.push("pause");
  }
  return [...new Set(base)];
}

function summarizeStep(stepKind: ActivityStep["stepKind"], attention: AttentionSignal): string {
...
async function emitStageEvent(
  recorder: ((event: ActivityStageEvent) => Promise<void>) | undefined,
  event: ActivityStageEvent,
): Promise<void> {
  if (!recorder) return;
  try {
