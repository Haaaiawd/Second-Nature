...
import type { RhythmPolicy, RhythmWindow, RhythmWindowDecision } from "./rhythm-policy.js";

function toMinuteOfDay(now: string, timezone: string): number {
...
function matchWindow(windows: RhythmWindow[], minuteOfDay: number): RhythmWindow {
  const found = windows.find((window) => minuteOfDay >= window.startMinute && minuteOfDay < window.endMinute);
  if (!found) {
    return windows[0] as RhythmWindow;
  }
  return found;
}

export function selectRhythmWindow(now: string, snapshot: ContinuitySnapshot, policy: RhythmPolicy): RhythmWindowDecision {
