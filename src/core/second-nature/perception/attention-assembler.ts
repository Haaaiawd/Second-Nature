...
function buildSignalId(cycleId: string, evidenceId: string, now: string): string {
  const hash = crypto
    .createHash("sha256")
...
export async function assembleAttention(
  input: AttentionAssemblerInput,
  deps: AttentionAssemblerDeps,
): Promise<AttentionAssemblerResult> {
  ...
  let status: AttentionSignal["status"] = "attentive";
