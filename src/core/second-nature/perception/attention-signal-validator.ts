...
import type { AttentionSignal, SourceRef } from "../../../shared/types/v9-contracts.js";
import {
  ATTENTION_SUMMARY_MAX_CHARS,
  POSSIBLE_ACTIONS_MAX_COUNT,
...
function hasWritableSources(sourceRefs: SourceRef[] | undefined): boolean {
  if (!sourceRefs || sourceRefs.length === 0) return false;
  return sourceRefs.every((ref) => typeof ref.id === "string" && ref.id.length > 0 && typeof ref.family === "string");
}

export function validateAttentionSignal(draft: AttentionSignal): ValidationResult {
  let signal = { ...draft };
  let blocked = false;
