...
export type DispatchResult =
  | { type: "connector"; request: ConnectorDispatchRequest }
  | { type: "guidance"; request: GuidanceDispatchRequest }
  | NoDispatchResult
  | GuidanceUnavailableResult
  | { type: "degraded"; degraded: DegradedOperationResult };
...
function inferGuidanceDraftType(
  actionKind: PlatformNeutralActionKind,
): GuidanceDispatchRequest["draftType"] {
  if (actionKind === "draft_reply" || actionKind === "auto_reply") return "reply";
  if (actionKind === "draft_publish" || actionKind === "auto_publish") return "publish";
  return "notify";
}
...
export function dispatchAllowedAction(
  proposal: ActionProposal,
  decision: ActionPolicyDecision,
  options?: DispatchAllowedActionOptions,
): DispatchResult {
