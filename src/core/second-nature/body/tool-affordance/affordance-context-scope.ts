...
function isWriteIntent(intent: string): boolean {
 ...
 */
export function applyAffordanceContextScope(
  items: readonly AffordanceItem[],
  scope: AffordanceContextScope = {},
): AffordanceItem[] {
  const allowedStatuses =
    scope.allowedStatuses && scope.allowedStatuses.length > 0
      ? scope.allowedStatuses
      : [...DEFAULT_ALLOWED_STATUSES];

  // blocked/pending_trust always excluded at the status level
  const effectiveAllowed = allowedStatuses.filter(
    (s) => !BLOCKED_STATUSES.includes(s),
  );

  const platformSet =
    scope.platformIds && scope.platformIds.length > 0
 ...
 */
export function defaultHeartbeatScope(): AffordanceContextScope {
  return {
