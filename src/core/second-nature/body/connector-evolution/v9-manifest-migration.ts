 ...
 */
export async function migrateV8ConnectorManifest(
  workspaceRoot: string,
  manifestPath: string,
  deps: ManifestMigrationDeps,
): Promise<ConnectorVersion | undefined> {
  const now = deps.now ?? (() => new Date().toISOString());
  const generateId = deps.generateId ?? defaultGenerateId;

  // Read manifest (JSON or YAML based on extension).
  let manifest: V8ConnectorManifest | undefined;
  if (manifestPath.endsWith(".json")) {
    manifest = await safeReadJson<V8ConnectorManifest>(join(workspaceRoot, manifestPath));
  } else if (manifestPath.endsWith(".yaml") || manifestPath.endsWith(".yml")) {
    manifest = await safeReadYaml<V8ConnectorManifest>(join(workspaceRoot, manifestPath));
  }
  if (!manifest || !manifest.platformId || !Array.isArray(manifest.capabilities)) return undefined;
  ...
  if (existingMigrated) return undefined;
