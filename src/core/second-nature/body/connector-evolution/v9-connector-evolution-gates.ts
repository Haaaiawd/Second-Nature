...
export interface GateDeps {
  ...
  checkAdapterSandboxSafety?: (adapterPath: string) => { safe: boolean; reason?: string };
  ...
  getWetProbeConfig?: (platformId: string) => { endpoint: string; capability: string } | undefined;
  ...
  checkCanaryHealth?: (platformId: string, versionId: string) => { healthy: boolean; reason?: string };
}
...
function pass(gate: string, evidenceRefs: SourceRef[] = [], reason?: string): GateResult {
  return { gate, passed: true, reason, evidenceRefs };
}

function fail(gate: string, reason: string, evidenceRefs: SourceRef[] = []): GateResult {
...
export function parseProposedChanges(plan: ConnectorEvolutionPlan): ProposedChanges {
  if (!plan.payloadJson) return {};
  try {
    const parsed = JSON.parse(plan.payloadJson);
    if (parsed && typeof parsed === "object" && !Array.isArray(parsed)) {
      return parsed as ProposedChanges;
    }
  } catch {
    // malformed payload
  }
 ...
 */
export async function runSchemaGate(
...
export async function runPermissionGate(
  ...
  if (deps.getAllowedPlatformCapabilities) {
    ...
    if (expanding.length > 0) {
      return fail(
        "permission",
        `capabilities_exceed_platform_scope: ${expanding.join(", ")}`,
        plan.sourceRefs,
      );
    }
  } else {
    ...
    if (misaligned.length > 0) {
      return fail(
        "permission",
        `capabilities_not_platform_scoped: ${misaligned.join(", ")}`,
        plan.sourceRefs,
      );
    }
  }
 ...
 */
export async function runWetProbeGate(
  version: ConnectorVersion,
  plan: ConnectorEvolutionPlan,
  deps: GateDeps,
): Promise<GateResult> {
 ...
 */
export async function runRollbackSetupGate(
  version: ConnectorVersion,
  plan: ConnectorEvolutionPlan,
  deps: GateDeps,
): Promise<GateResult> {
  ...
  if (deps.getPreviousStableVersionId) {
    ...
    if (!previousId || previousId !== version.previousStableRef) {
      return fail(
        "rollback_setup",
        "previous_stable_ref_not_resolvable",
        plan.sourceRefs,
      );
    }
  }
...
export const GATE_RUNNERS: Record<string, GateRunner> = {
