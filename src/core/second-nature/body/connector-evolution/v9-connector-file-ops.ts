...
export async function acquireFileLock(
  filePath: string,
  timeoutMs: number = CONNECTOR_ASSET_FILE_LOCK_TIMEOUT_MS,
 ...
 */
export async function safeReadYaml<T>(filePath: string): Promise<T | undefined> {
  try {
    const content = await fs.readFile(filePath, "utf-8");
 ...
 */
export async function rollbackConnectorFiles(
  currentAssets: ConnectorAssetPaths,
  previousAssets: ConnectorAssetPaths,
  workspaceRoot: string,
  ...
  const assetPairs: [string | undefined, string | undefined][] = [
  ...
  for (const [currentPath, previousPath] of assetPairs) {
    if (!currentPath || !previousPath) {
      if (currentPath) skipped.push(currentPath);
      continue;
    }
