...
export type CapabilityClass =
  | "consume"   // read-only browsing / notification reading
  | "broadcast" // primary outward publishing
  | "interact"  // reply / DM / comment
  | "discover"  // research / work discovery
  | "claim";    // task claim
 ...
 */
export function inferCapabilityClass(capabilityIntent: string): CapabilityClass | null {
  if (!capabilityIntent || typeof capabilityIntent !== "string") return null;
...
export const CAPABILITY_CLASS_SCENE_MAP: Readonly<Record<CapabilityClass, string>> = {
  consume:   "explore",
  discover:  "explore",
  broadcast: "social",
  interact:  "reply",
