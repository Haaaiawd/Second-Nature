...
import type { RelationshipMemory, ResponsePatternEntry } from "./channel-feedback-ingestion-service.js";
...
export interface FallbackCopy {
  text: string;
  hasInformationValue: boolean; // always true; guard to prevent empty string
  sourceAnchor?: string;        // concrete anchor if available
...
const DRY_FILLER_PATTERNS: RegExp[] = [
 ...
 */
export function runStyleLint(draftText: string): StyleLintResult {
  const violations: StyleLintViolation[] = [];
  const hitRules: StyleLintRule[] = [];
 ...
 */
export function buildFallbackCopy(ctx: FallbackContext): FallbackCopy {
...
export function computeFrequency(memory: RelationshipMemory): OutreachFrequency {
  ...
  if (hasBlock) return "paused";

  // Trust-based floor
  if (memory.trustDelta <= TRUST_MINIMAL) return "minimal";
  if (memory.trustDelta <= TRUST_REDUCED) return "reduced";

  // No-reply signal override
  if (noReplyRatio >= NO_REPLY_RATIO_THRESHOLD && recent.length >= 2) return "reduced";
...
export function computeStyle(memory: RelationshipMemory, frequency?: OutreachFrequency): OutreachStyle {
  ...
  const recent = memory.responsePatterns.slice(-RECENT_PATTERNS_WINDOW);
  const positiveCount = recent.filter((p: ResponsePatternEntry) => p.tone === "positive").length;
  const negativeCount = recent.filter((p: ResponsePatternEntry) => p.tone === "negative").length;

  if (positiveCount > negativeCount && positiveCount >= 2) return "warm_anchored";
  if (negativeCount > positiveCount) return "light_check";
...
function buildRationale(
  ...
  const hasBlock = memory.responsePatterns.some((p) => p.reaction === "block");
  if (hasBlock) parts.push("block_detected");

  const recent = memory.responsePatterns.slice(-RECENT_PATTERNS_WINDOW);
  const noReplyCount = recent.filter(
    (p: ResponsePatternEntry) => p.reaction === "ignore",
  ).length;
  if (noReplyCount > 0) parts.push(`no_reply_signals=${noReplyCount}`);
...
export function selectOutreachStrategy(
  memory: RelationshipMemory,
  options?: OutreachStrategySelectorOptions,
): OutreachStrategy {
