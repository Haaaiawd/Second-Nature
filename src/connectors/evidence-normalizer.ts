...
import {
  ...
  type EvidenceContentStatus,
  type EvidenceContentMissingReason,
...
async function normalizeLegacyConnectorEvidence(
  ...
  for (const item of items) {
    ...
    const contentStatus: EvidenceContentStatus = looksLikeIdOnly || trimmedContent.length === 0
      ? "content_missing"
      : "content_present";
    const contentMissingReason: EvidenceContentMissingReason | undefined =
    ...
    const normalized: NormalizedEvidenceContent = {
      schemaVersion: 1,
      sourceKind: "unknown",
      platformId: result.platformId,
      capabilityId: result.capabilityId,
