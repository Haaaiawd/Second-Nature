...
import type { CapabilityProbeResult, ProbeActualStatus } from "../../shared/types/v7-entities.js";
...
export interface WetProbeRunner {
  runWetProbe(
    ...
    options?: { httpGet?: HttpGetFn },
  ): Promise<WetProbeResult>;
}

const PROBE_POLICY_DENIED: WetProbeResult = {
  probeResult: {
...
function resolveProbeConfig(
  registry: CapabilityContractRegistryV7,
  platformId: string,
  capabilityId: string,
): { safeEndpoint: string; idempotencyClass: string } | undefined {
  const resolved = registry.resolveCapability(
...
function actualStatusFromHttpStatus(status: number): ProbeActualStatus {
  if (status >= 200 && status < 300) return "available";
  if (status === 429 || status === 503) return "degraded";
  return "unavailable";
}

export function createWetProbeRunner(): WetProbeRunner {
  const defaultHttpGet: HttpGetFn = async (url) => {
