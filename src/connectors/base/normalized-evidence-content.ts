...
function findFirstValue(obj: unknown, keys: string[]): unknown {
...
function findDeepValue(obj: unknown, keys: string[]): unknown {
...
function inferSourceKind(item: Record<string, unknown>): EvidenceSourceKind {
  const typeHint = String(findFirstValue(item, ["type", "kind", "sourceKind", "object", "category"]) ?? "").toLowerCase();
  if (typeHint.includes("post")) return "post";
  if (typeHint.includes("comment") || typeHint.includes("reply")) return "comment";
  if (typeHint.includes("profile") || typeHint.includes("user") || typeHint.includes("author")) return "profile";
  if (typeHint.includes("task") || typeHint.includes("issue") || typeHint.includes("todo")) return "task";
  if (typeHint.includes("event") || typeHint.includes("activity")) return "event";
  if (typeHint.includes("game") || typeHint.includes("match") || typeHint.includes("score")) return "game_state";
  if (typeHint.includes("notification") || typeHint.includes("alert")) return "notification";
  if (typeHint.includes("document") || typeHint.includes("article") || typeHint.includes("page")) return "document";
  return "unknown";
}
...
function extractBodyText(item: Record<string, unknown>): string | undefined {
  return firstNonEmptyString([
...
function extractActor(item: Record<string, unknown>): EvidenceActor | undefined {
  const author = findFirstValue(item, ["author", "creator", "user", "actor", "from"]);
  if (!isRecord(author) && !isNonEmptyString(author)) return undefined;
...
function extractOccurredAt(item: Record<string, unknown>): string | undefined {
...
function extractEntities(item: Record<string, unknown>): string[] {
  const entities = findFirstValue(item, ["entities", "mentions", "hashtags", "keywords"]);
  return toStringArray(entities);
}

function extractMetrics(item: Record<string, unknown>): Record<string, number | string | boolean> | undefined {
...
): NormalizedEvidenceContent | null {
 ...
 */
export function extractNormalizedEvidenceItems(
  data: unknown,
  options: ExtractEvidenceOptions,
): NormalizedEvidenceContent[] {
  const items = extractItems(data);
  const out: NormalizedEvidenceContent[] = [];
 ...
 */
export function computeEvidenceContentHash(content: NormalizedEvidenceContent): string {
  return computeEvidenceContentHashSync(content);
}

import * as crypto from "node:crypto";

export function computeEvidenceContentHashSync(content: NormalizedEvidenceContent): string {
  const canonical = [
    content.platformId,
