...
import type {
  ...
  HostCapabilityConflictRecord,
  HostCapabilityProbeOptions,
  HostCapabilityReport,
  SourceRef,
} from "./types.js";

function mergeEvidenceRefs(...groups: SourceRef[][]): SourceRef[] {
...
export function probeHostCapability(options: HostCapabilityProbeOptions): HostCapabilityReport {
  ...
  const conflictRecords: HostCapabilityConflictRecord[] = [];
  ...
  const evidenceRefs = mergeEvidenceRefs(
    pluginLoad.evidenceRefs,
    heartbeatBridge.evidenceRefs,
    heartbeatToolInvocation.evidenceRefs,
    delivery.evidenceRefs,
    ackDropBehavior.evidenceRefs,
    ...hookSupport.map((h) => h.evidenceRefs)
  );
