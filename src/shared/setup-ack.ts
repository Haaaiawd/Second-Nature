...
export type ValidateSetupAckResult =
  | SetupAckValidationResult
  | { ok: false; errors: SetupAckValidationError[] };
...
const VALID_WRITERS: ReadonlySet<string> = new Set([
  "setup_ack_command",
  "host_setup_bridge",
]);

export function validateSetupAck(
  raw: Record<string, unknown>,
): ValidateSetupAckResult {
  const errors: SetupAckValidationError[] = [];
  ...
  return {
    ...
    ack: {
      ...
      placedIn: placedIn as SetupAckPlacement,
      placementProofRef: placementProofRef!,
      writer: writer as SetupAckWriter,
 ...
 */
export function isSetupAckComplete(
  raw: Record<string, unknown>,
): { complete: true; ack: SetupAck } | { complete: false; errors: SetupAckValidationError[] } {
