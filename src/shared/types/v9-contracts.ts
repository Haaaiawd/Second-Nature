...
// Minimal EvidenceItem surface used by identity normalization in v9.
// Full EvidenceItem remains owned by v8 ingestion; v9 identity port consumes
// the platform/content fields needed for deduplication.
...
export type CharacterSignalKind =
  | "tool_experience"
  | "action_closure"
  | "owner_feedback"
  | "relationship_signal"
...
// ───────────────────────────────────────────────────────────────
// 7b. Connector Evolution Engine contracts (T6.3.1)
// ───────────────────────────────────────────────────────────────
...
export const PRE_ACTIVATION_GATES: readonly EvolutionGateName[] = [
  "schema",
  "permission",
  "sandbox",
  "fixture",
...
export type AutonomousChangeKind =
  | "routine_install"
  | "routine_supersede"
  | "routine_retire"
...
export type AutonomousChangeStatus =
  | "proposed"
  | "gated"
  | "activated"
  | "rolled_back"
...
// ───────────────────────────────────────────────────────────────
// action-closure-policy-system.detail.md §2.3 cross-system input/output contracts
// ───────────────────────────────────────────────────────────────
...
// `ActionClosureRecord` defined in §9.
...
// ───────────────────────────────────────────────────────────────
// observability-recovery-system.detail.md §2.1 storage/read-model contracts
// ───────────────────────────────────────────────────────────────
...
export type CharacterFrameEventKind =
  | "refresh"
  | "accepted"
  | "rejected"
  | "revised"
...
export interface ToolRoutineRegistrySnapshot {
  ...
  routines: {
    ...
    rollbackRef?: string;
    healthReason?:
      | "routine_permission_expansion_denied"
      | "routine_guard_validation_failed"
      | "routine_install_pending";
    sourceRefs: SourceRef[];
  }[];
}
...
// Result type used by continuity-card assembly before health aggregation.
...
// ───────────────────────────────────────────────────────────────
// observability-recovery-system.detail.md §3.2 health/digest contracts
// ───────────────────────────────────────────────────────────────
...
export type V9ReasonCode =
  // Attention / Activity
  | "attention_hint_without_agent_or_routine_intent"
  | "attention_blocked_missing_sources"
  | "activity_thread_stale"
  | "activity_thread_overlong"
  | "activity_thread_missing_closure"
  | "activity_thread_blocked"
  // Continuity
  | "continuity_unavailable"
  | "continuity_stale_projections"
  // Character
  ...
  | "closure_downgraded"
  // Routine / policy
  | "routine_guard_schema_invalid"
  | "routine_permission_expansion_denied"
  | "routine_guard_policy_denied"
  | "routine_validation_pending"
  | "routine_invocation_denied"
  // Connector evolution
  | "evolution_gate_schema_failed"
  | "evolution_gate_permission_failed"
  | "evolution_gate_sandbox_failed"
  | "evolution_gate_fixture_failed"
  | "evolution_gate_wet_probe_failed"
  | "evolution_canary_failed"
  | "evolution_rollback_failed"
  // Ledger / redaction
  | "ledger_missing_source_refs"
  | "ledger_redaction_blocked"
  // Legacy compatibility
  | "v8_legacy_judgment_mapped"
  // Loop / observability
  | "loop_healthy"
  | "loop_degraded_missing_closure"
  | "loop_blocked_rollback_failed"
