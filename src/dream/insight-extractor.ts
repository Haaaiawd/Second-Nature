...
function groupByDay(items: Array<{ id: string; summary: string; createdAt: string }>): Map<string, string[]> {
...
function findRecurringThemes(items: Array<{ id: string; summary: string; createdAt: string }>): Array<{
  theme: string;
  count: number;
  sourceIds: string[];
}> {
  const themeCounts = new Map<string, { count: number; sourceIds: string[] }>();
  for (const item of items) {
...
export function extractInsights(input: ExtractInsightsInput): {
  insights: DreamInsight[];
  unsupportedClaims: string[];
} {
  const allItems = [
    ...input.evidenceSummaries.map((e) => ({ ...e, origin: "evidence" as const })),
    ...input.chronicleSummaries.map((c) => ({ ...c, origin: "chronicle" as const })),
  ];
  ...
  const learningItems = allItems.filter(
    (item) => countKeywordMatches(item.summary, LEARNING_KEYWORDS) > 0,
  );
  ...
  const conflictItems = allItems.filter(
    (item) => countKeywordMatches(item.summary, CONFLICT_KEYWORDS) > 0,
  );
