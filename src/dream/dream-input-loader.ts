...
import type { DreamInputBundle, DreamStatePort, ToolExperienceSummary } from "./types.js";
...
function extractRefIdsFromJson(json: string): string[] {
  ...
  for (const item of parsed) {
    if (typeof item === "string") {
      ...
      if (typeof obj.sourceId === "string") ids.push(obj.sourceId);
    }
  }
  return ids;
}
...
function extractConsumedRefIdsFromEntriesJson(json: string): string[] {
  const entries = safeParseJson<unknown[]>(json, []);
...
export function createDreamInputLoader(
  options: DreamInputLoaderOptions,
): Pick<DreamStatePort, "loadDreamInputs"> {
  ...
  return {
    ...
    async loadDreamInputs(query = {}): Promise<DreamInputBundle> {
      ...
      const diaryResult = sqlite.exec(
        `SELECT source_refs_json FROM daily_diary_index WHERE created_at >= ? ORDER BY created_at DESC`,
        [since],
      );
      ...
      const evidenceResult = sqlite.exec(
        `SELECT source_refs_json FROM life_evidence_index WHERE timestamp >= ? ORDER BY timestamp DESC LIMIT ?`,
        [since, evidenceLimit],
      );
      ...
      const acceptedResult = sqlite.exec(
        `SELECT canonical_entries_json FROM dream_output_index WHERE status = 'accepted'`,
      );
      ...
      const evidenceRefs = [...candidateRefs].filter(
        (ref) => !consumedRefs.has(ref),
      );

      // ─── 5. Load ToolExperience summaries (aggregated by connector/capability/outcome) ─
      const toolExpResult = sqlite.exec(
        `SELECT connector_id, capability_id, outcome, COUNT(*) as count, MAX(created_at) as last_recorded_at
      ...
      const toolExperienceSummaries: ToolExperienceSummary[] = [];
